      ...
      <div style="margin-top: 10px">
        <el-table :data="tableData" stripe style="width: 100%">
          ...
          <el-table-column prop="count" label="购买数量">
            <template slot-scope="scope">
              <el-input-number v-model="scope.row.count" :min="1" :max="10" label="购买数量" style="width: 100px"
          ...
          <div style="display: flex; background-color: white; padding: 10px">
            ...
            <div style="flex: 1;color: red; text-align: right">
              <div>
                ...
                <el-button class="cartBtn" @click="submitOrder">去结算</el-button>
              </div>
  ...
  methods: {
    ...
    load() {
      API.get("/api/cart").then(res => {
        ...
        if (this.tableData) {
          this.count = this.tableData.map(v => v.count).reduce((a, b) => {
            return a + b
          },0)
...
.cartBtn:hover {
  background-color: rgba(242, 88, 7, 0.5);
}
</style>
